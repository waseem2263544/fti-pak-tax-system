...
<div class="card">
    <div class="card-header d-flex align-items-center gap-2">
        ...
        Edit Account: {{ $account->code }} - {{ $account->name }}
    </div>
        ...
        <form action="{{ route('accounting.accounts.update', $account) }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control @error('code') is-invalid @enderror" name="code" value="{{ old('code', $account->code) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select searchable @error('parent_id') is-invalid @enderror" name="parent_id" id="parent-account">
                        ...
                        @foreach($parentAccounts ?? [] as $pa)
                            <option value="{{ $pa->id }}" data-type="{{ $pa->type }}" {{ old('parent_id', $account->parent_id) == $pa->id ? 'selected' : '' }}>
                                {{ $pa->code }} - {{ $pa->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" name="description" rows="3">{{ old('description', $account->description) }}</textarea>
            ...
            <div class="row g-3 mt-1">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control @error('opening_balance') is-invalid @enderror" name="opening_balance" value="{{ old('opening_balance', $account->opening_balance ?? '0.00') }}" step="0.01" min="0">
                ...
                <div class="col-md-6">
                    ...
                    <div class="d-flex gap-3 mt-2">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="opening_balance_type" id="balance-debit" value="debit" {{ old('opening_balance_type', $account->opening_balance_type ?? 'debit') == 'debit' ? 'checked' : '' }}>
                            <label class="form-check-label" for="balance-debit" style="font-size: 0.85rem;">Debit</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="opening_balance_type" id="balance-credit" value="credit" {{ old('opening_balance_type', $account->opening_balance_type ?? '') == 'credit' ? 'checked' : '' }}>
            ...
            <div class="d-flex gap-2 mt-4 pt-3" style="border-top: 1px solid #f0f2f5;">
                <button type="submit" class="btn btn-accent"><i class="bi bi-check-lg me-1"></i> Update Account</button>
    ...
    function updateSubTypes() {
        ...
        if (type && subTypes[type]) {
            subTypes[type].forEach(function(st) {
                ...
                opt.value = st;
                opt.textContent = st;
                if (st === currentSubType) opt.selected = true;
