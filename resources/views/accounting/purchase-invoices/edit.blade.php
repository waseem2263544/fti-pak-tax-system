...
<form action="{{ route('accounting.purchase-invoices.update', $purchaseInvoice) }}" method="POST" id="bill-form">
    @csrf
    @method('PUT')
    ...
    <div class="card mb-4">
        <div class="card-header d-flex align-items-center gap-2">
            ...
            Edit Bill: {{ $purchaseInvoice->bill_number }}
        </div>
        <div class="card-body" style="padding: 24px;">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select class="form-select searchable @error('contact_id') is-invalid @enderror" name="contact_id" id="vendor-select">
                        ...
                        @foreach($contacts ?? [] as $contact)
                            <option value="{{ $contact->id }}" {{ old('contact_id', $purchaseInvoice->contact_id) == $contact->id ? 'selected' : '' }}>{{ $contact->name }}</option>
                        @endforeach
                    </select>
                    @error('contact_id') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    <div class="mt-2">
                        <input type="text" class="form-control form-control-sm @error('vendor_name') is-invalid @enderror" name="vendor_name" value="{{ old('vendor_name', $purchaseInvoice->vendor_name) }}" placeholder="Or enter vendor name manually..." style="font-size: 0.82rem;">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" class="form-control @error('vendor_invoice_number') is-invalid @enderror" name="vendor_invoice_number" value="{{ old('vendor_invoice_number', $purchaseInvoice->vendor_invoice_number) }}" placeholder="Vendor's bill #">
                    @error('vendor_invoice_number') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>
                <div class="col-md-2">
                    <label class="form-label">Bill Date <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('bill_date') is-invalid @enderror" name="bill_date" value="{{ old('bill_date', $purchaseInvoice->bill_date->format('Y-m-d')) }}" required>
                    @error('bill_date') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>
                <div class="col-md-2">
                    <label class="form-label">Due Date <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('due_date') is-invalid @enderror" name="due_date" value="{{ old('due_date', $purchaseInvoice->due_date->format('Y-m-d')) }}" required>
                    @error('due_date') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>
                <div class="col-md-2">
                    <label class="form-label">Reference</label>
                    <input type="text" class="form-control @error('reference') is-invalid @enderror" name="reference" value="{{ old('reference', $purchaseInvoice->reference) }}" placeholder="PO number...">
            ...
            <div class="card mb-4">
                <div class="card-body" style="padding: 20px;">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="notes" rows="3">{{ old('notes', $purchaseInvoice->notes) }}</textarea>
                    </div>
                    <div>
                        <label class="form-label">Terms &amp; Conditions</label>
                        <textarea class="form-control" name="terms" rows="3">{{ old('terms', $purchaseInvoice->terms) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body" style="padding: 20px;">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3" style="padding-bottom: 12px; border-bottom: 1px solid #f0f2f5;">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <input type="number" class="form-control form-control-sm" name="discount" id="discount-input" value="{{ old('discount', $purchaseInvoice->discount ?? 0) }}" step="0.01" min="0" style="width: 100px;" oninput="calculateTotals()">
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addLine(accountId, description, qty, price, taxRate) {
        lineIndex++;
        qty = qty || 1;
        price = price || 0;
        taxRate = taxRate || 0;
        ...
        row.innerHTML = `
            ...
            <td>
                <input type="text" class="form-control form-control-sm" name="items[${lineIndex}][description]" value="${description || ''}">
            </td>
            <td>
                <input type="number" class="form-control form-control-sm text-end inv-qty" name="items[${lineIndex}][quantity]" value="${qty}" step="0.01" min="0.01" oninput="calculateRow(this)" onfocus="this.select()">
            </td>
            <td>
                <input type="number" class="form-control form-control-sm text-end inv-price" name="items[${lineIndex}][unit_price]" value="${parseFloat(price).toFixed(2)}" step="0.01" min="0" oninput="calculateRow(this)" onfocus="this.select()">
            </td>
            <td>
                <input type="number" class="form-control form-control-sm text-end inv-tax" name="items[${lineIndex}][tax_rate]" value="${taxRate}" step="0.01" min="0" max="100" oninput="calculateRow(this)" onfocus="this.select()">
            </td>
            <td>
                <input type="text" class="form-control form-control-sm text-end inv-amount" readonly style="background: #fafbfc; font-weight: 600;" value="${amount.toFixed(2)}">
                <input type="hidden" name="items[${lineIndex}][amount]" class="inv-amount-hidden" value="${amount.toFixed(2)}">
            </td>
        ...
        if (typeof TomSelect !== 'undefined') {
            ...
            if (accountId) ts.setValue(accountId);
        }
    ...
    addLine('{{ $item->account_id }}', '{{ addslashes($item->description ?? '') }}', {{ $item->quantity }}, {{ $item->unit_price }}, {{ $item->tax_rate ?? 0 }});
    @endforeach

    // If no items exist, add an empty line
