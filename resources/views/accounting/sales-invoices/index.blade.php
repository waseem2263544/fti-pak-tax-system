...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <p style="color: #9ca3af; font-size: 0.85rem; margin: 0;">Manage sales invoices and track payments.</p>
    </div>
    <a href="{{ route('accounting.sales-invoices.create') }}" class="btn btn-accent"><i class="bi bi-plus-lg me-1"></i> New Invoice</a>
</div>

<!-- Filters -->
<div class="card mb-4">
    <div class="card-body" style="padding: 16px 20px;">
        <form method="GET" action="{{ route('accounting.sales-invoices.index') }}">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-1">
                    <a href="{{ route('accounting.sales-invoices.index') }}" class="btn btn-outline-primary w-100" title="Clear"><i class="bi bi-x-lg"></i></a>
                </div>
                @endif
            </div>
        </form>
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <a href="{{ route('accounting.sales-invoices.show', $invoice) }}" style="color: var(--primary); font-weight: 600; text-decoration: none; font-size: 0.88rem;">
                            {{ $invoice->invoice_number }}
                        </a>
                    </td>
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            <div style="width: 28px; height: 28px; background: rgba(48,58,80,0.06); border-radius: 6px; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.6rem; color: var(--primary);">{{ strtoupper(substr($invoice->client->name ?? '', 0, 2)) }}</div>
                            <span style="font-size: 0.85rem;">{{ Str::limit($invoice->client->name ?? '-', 25) }}</span>
                        </div>
                    </td>
                    <td style="font-size: 0.85rem; color: #6b7280;">{{ $invoice->invoice_date->format('M d, Y') }}</td>
                    ...
                    <td class="text-end">
                        <div class="d-flex gap-1 justify-content-end">
                            <a href="{{ route('accounting.sales-invoices.show', $invoice) }}" class="btn btn-sm btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>
                            @if($invoice->status === 'draft')
                            <a href="{{ route('accounting.sales-invoices.edit', $invoice) }}" class="btn btn-sm btn-outline-primary" title="Edit"><i class="bi bi-pencil"></i></a>
                            @endif
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="9" class="text-center py-5" style="color: #9ca3af;">
                        ...
                        No invoices found. <a href="{{ route('accounting.sales-invoices.create') }}" style="color: var(--primary); font-weight: 600;">Create one</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
