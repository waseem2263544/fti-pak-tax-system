...
<form action="{{ route('accounting.payment-vouchers.store') }}" method="POST" id="payment-form">
    @csrf
        ...
        <div class="col-md-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body" style="padding: 24px;">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Pay To (Vendor) <span class="text-danger">*</span></label>
                            <select class="form-select searchable @error('contact_id') is-invalid @enderror" name="contact_id" id="vendor-select" required>
                    ...
                    <div class="row g-3 mt-1">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Link to Bill</label>
                            <select class="form-select searchable @error('bill_id') is-invalid @enderror" name="bill_id" id="bill-select">
                                <option value="">No bill link</option>
                                @foreach($unpaidBills ?? [] as $bill)
                                    <option value="{{ $bill->id }}" {{ old('bill_id', request('bill_id')) == $bill->id ? 'selected' : '' }}>
                                        {{ $bill->bill_number }} - {{ $bill->contact->name ?? '' }} (PKR {{ number_format($bill->total - $bill->paid_amount, 2) }})
                                    </option>
                                @endforeach
                            </select>
                            @error('bill_id') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>
                    </div>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('accounting.payment-vouchers.index') }}" class="btn btn-outline-primary">Cancel</a>
            </div>
        </div>
            ...
            <div class="card" style="position: sticky; top: 24px;">
                ...
                <div class="card-body" style="padding: 20px;">
                    <div style="text-align: center; padding: 20px 0;">
                        <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #9ca3af; margin-bottom: 8px;">Payment Amount</div>
                        <div style="font-weight: 800; font-size: 2rem; color: #ef4444;" id="summary-amount">PKR 0.00</div>
                    </div>
                    <div style="border-top: 1px solid #f0f2f5; padding-top: 16px;">
                        <div class="d-flex justify-content-between mb-2">
                            <span style="font-size: 0.82rem; color: #9ca3af;">Type</span>
                            <span style="font-size: 0.85rem; font-weight: 600; color: var(--primary);">Payment</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
