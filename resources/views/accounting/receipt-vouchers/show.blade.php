@extends('layouts.app')
@section('title', 'Receipt ' . $voucher->voucher_number)
...
<div class="d-flex justify-content-between align-items-center mb-4 no-print">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('accounting.receipt-vouchers.pdf', $receiptVoucher) }}" target="_blank" class="btn btn-outline-primary btn-sm"><i class="bi bi-file-earmark-pdf me-1"></i> PDF</a>
...
<div class="card" style="max-width: 750px; margin: 0 auto;">
    <div class="card-body" style="padding: 48px;">
        ...
        <div class="d-flex justify-content-between align-items-start mb-5">
            <div>
                <h3 style="font-weight: 800; color: var(--primary); margin: 0;">RECEIPT VOUCHER</h3>
        ...
        <div class="row g-4 mb-5">
            <div class="col-md-6">
                <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #9ca3af; margin-bottom: 8px;">Received From</div>
                <div style="font-weight: 700; font-size: 1rem; color: var(--primary);">{{ $voucher->client->name ?? $voucher->party_name ?? '-' }}</div>
                @if($voucher->client->email ?? null)
                    <div style="font-size: 0.85rem; color: #6b7280;">{{ $voucher->client->email }}</div>
                @endif
            </div>
            <div class="col-md-6">
                <div class="row g-3">
        ...
        <!-- Amount Card -->
        <div style="background: rgba(16,185,129,0.04); border: 1px solid rgba(16,185,129,0.15); border-radius: 12px; padding: 28px; text-align: center; margin-bottom: 32px;">
            <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #9ca3af; margin-bottom: 8px;">Amount Received</div>
            <div style="font-weight: 800; font-size: 2.2rem; color: #10b981;">PKR {{ number_format($voucher->amount, 2) }}</div>
        </div>
        ...
        <div class="row g-3 mb-4">
            ...
            @endif
            <div class="col-md-4">
                <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #9ca3af; margin-bottom: 4px;">Receiving Account</div>
        ...
        @if($voucher->invoice)
        <div class="mb-4">
            <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #9ca3af; margin-bottom: 4px;">Linked Invoice</div>
            <a href="{{ route('accounting.sales-invoices.show', $voucher->invoice) }}" style="font-size: 0.9rem; font-weight: 600; color: var(--primary); text-decoration: none;">
                {{ $voucher->invoice->invoice_number }} <i class="bi bi-box-arrow-up-right" style="font-size: 0.75rem;"></i>
            </a>
        </div>
        @endif
