...
<div class="mb-4">
    <form method="GET" action="{{ route('search') }}">
        <div style="position: relative; max-width: 600px;">
            <i class="bi bi-search" style="position: absolute; left: 16px; top: 50%; transform: translateY(-50%); color: var(--n-400);"></i>
...
@if(strlen($q) >= 2)
<p style="color: var(--n-500); font-size: 0.85rem; margin-bottom: 20px;">
...
<div class="card mb-4">
    ...
    <div class="p-0">
        ...
        <a href="{{ route('tasks.show', $task) }}" class="d-flex justify-content-between align-items-center px-4 py-3 text-decoration-none" style="border-bottom: 1px solid var(--n-50); color: var(--primary);">
            <div>
                <div style="font-weight: 600;">{{ $task->title }}</div>
                <div style="font-size: 0.75rem; color: var(--n-400);">{{ $task->client->name ?? '' }} {{ $task->due_date ? '· Due ' . $task->due_date->format('M d') : '' }}</div>
            </div>
            <span class="badge" style="background: {{ $task->status == 'completed' ? 'var(--ok-tint)' : ($task->status == 'overdue' ? 'var(--danger-tint)' : 'var(--warn-tint)') }}; color: {{ $task->status == 'completed' ? 'var(--ok-ink)' : ($task->status == 'overdue' ? 'var(--danger)' : 'var(--warn-ink)') }};">{{ ucfirst($task->status) }}</span>
        </a>
        @endforeach
    </div>
</div>
...
<div class="card mb-4">
    ...
    <div class="p-0">
        ...
        <a href="{{ route('proceedings.show', $proc) }}" class="d-flex justify-content-between align-items-center px-4 py-3 text-decoration-none" style="border-bottom: 1px solid var(--n-50); color: var(--primary);">
            <div>
                <div style="font-weight: 600;">{{ $proc->title }}</div>
                <div style="font-size: 0.75rem; color: var(--n-400);">{{ $proc->client->name ?? '' }} · {{ $proc->section ?? '' }} · {{ $proc->tax_year ?? '' }}</div>
            </div>
            <span class="badge" style="background: var(--info-tint); color: var(--info-ink);">{{ str_replace('_', ' ', ucfirst($proc->stage)) }}</span>
        </a>
        @endforeach
    </div>
</div>
...
<div class="card mb-4">
    ...
    <div class="p-0">
        ...
        <a href="{{ route('processes.show', $proc) }}" class="d-flex justify-content-between align-items-center px-4 py-3 text-decoration-none" style="border-bottom: 1px solid var(--n-50); color: var(--primary);">
            <div>
                <div style="font-weight: 600;">{{ $proc->title }}</div>
...
<div class="card mb-4">
    ...
    <div class="p-0">
        ...
        <div class="d-flex justify-content-between align-items-center px-4 py-3" style="border-bottom: 1px solid var(--n-50);">
            <div>
                <div style="font-weight: 600; color: var(--primary);">{{ $notice->subject }}</div>
                <div style="font-size: 0.75rem; color: var(--n-400);">{{ $notice->client->name ?? 'Unassigned' }} · {{ $notice->notice_section }} · {{ $notice->tax_year }}</div>
            </div>
        </div>
        @endforeach
    </div>
...
<div class="card mb-4">
    ...
    <div class="p-0">
        ...
        <a href="{{ $client->sharePointUrl }}" target="_blank" class="d-flex justify-content-between align-items-center px-4 py-3 text-decoration-none" style="border-bottom: 1px solid var(--n-50); color: var(--primary);">
            <div class="d-flex align-items-center gap-3">
                <i class="bi bi-folder-fill" style="color: var(--info); font-size: 1.1rem;"></i>
                <div>
                    <div style="font-weight: 600;">{{ $client->name }}</div>
                    <div style="font-size: 0.72rem; color: var(--n-400);">{{ Str::limit($client->folder_link, 50) }}</div>
                </div>
            </div>
            <i class="bi bi-box-arrow-up-right" style="color: var(--n-400);"></i>
        </a>
        @endforeach
    </div>
</div>
...
<div class="card">
    <div class="card-body text-center py-5">
        <i class="bi bi-search" style="font-size: 2.5rem; color: var(--n-150);"></i>
        <h5 class="mt-3" style="color: var(--primary);">No results found</h5>
        <p style="color: var(--n-400); font-size: 0.85rem;">Try a different search term</p>
    </div>
</div>
@endif
...
<div class="card">
    <div class="card-body text-center py-5">
        <i class="bi bi-search" style="font-size: 2.5rem; color: var(--n-150);"></i>
        <p style="color: var(--n-400); font-size: 0.85rem; margin-top: 12px;">Type at least 2 characters to search</p>
    </div>
</div>
@endif
@endsection
